...
const handleErrorToast = () => {
  return MySwal.fire({
    title: "Error!",
    text: "Edit collection không thành công!",
    icon: "error",
    customClass: {
...
const handleSuccessToast = () => {
  return MySwal.fire({
    title: "Success!",
    text: "Edit collection thành công.",
    icon: "success",
    customClass: {
      confirmButton: "btn btn-primary",
    },
...
const EditCollectionModal = (props) => {
  ...
  useEffect(() => {
    const getTags = async () => {
      ...
      let arr = res.data.filter((t) => t.type == 1);
  ...
  const onSubmit = async (dataForm) => {
    ...
    try {
      ...
      console.log(tagSelected.map((tag) => tag.value));
      //** Set data push */
      dataForm["url_thumbnail"] = url_thumbnail || data.url_thumbnail;
      dataForm["tags"] = tagSelected.map((tag) => tag.value);
      dataForm["status"] = checkedStatus ? 1 : 0;

      console.log(dataForm);

      if (type == 1) {
        await Api.postCollection(dataForm)
      ...
      if (type == 2) {
        await Api.updateCollectionByID(dataForm.id, dataForm)
          .then((res) => handleSuccessToast())
  ...
  const onDelete = async () => {
    await Api.deleteCollectionByID(data.id)
      .then((res) => handleSuccessToast())
      .catch((error) => handleErrorToast());
